...
if uploaded is not None:
    ...
    df["CTR_score_full"] = df.apply(
        lambda r: piecewise_score(
            r["CTR"],
            CTR_THRESHOLDS.get(r.get("渠道"), CTR_UNKNOWN_THRESHOLD),
            CTR_AMPS.get(r.get("渠道"), CTR_AMP_UNKNOWN)
        ),
        axis=1
    )
    df["GC_score_full"] = df.apply(
        lambda r: piecewise_score(
            r["订单GC转化率"],
            GC_THRESHOLDS.get(r.get("渠道"), GC_UNKNOWN_THRESHOLD),
            GC_AMPS.get(r.get("渠道"), GC_AMP_UNKNOWN)
        ),
        axis=1
    )
    df["综合评分_full"] = (
